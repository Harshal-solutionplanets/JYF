...
const GalleryArea = () => {
    ...
    const sortedGalleryItems = React.useMemo(() => {
        if (categories.length === 0) return galleryItems;

        // Create a map for quick priority lookup based on categories array order
        const categoryOrder = {};
        ...
        return [...galleryItems].sort((a, b) => {
            ...
            // If category matches, use category priority
            const pA = orderA !== undefined ? orderA : 999;
            const pB = orderB !== undefined ? orderB : 999;

            if (pA !== pB) return pA - pB;

            // If same category or both unknown, maintain relative order (already sorted by created_at desc)
            return 0;
        });
    }, [galleryItems, categories]);

    const filteredItems = sortedGalleryItems.filter(item =>
